...
func _ready() -> void:
	...
	if (tile_coords.x % 2 == 0 and tile_coords.y % 2 == 0) or (tile_coords.x % 2 == 1 and tile_coords.y % 2 == 1):
		sprite.texture = load("res://minigames/chess/tile_dark.png")
...
func fall():
	var tween = create_tween()
	tween.tween_property(self, "global_position", global_position + Vector2(0, 1000), 1).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN)
	tween.play()
